...
describe('lib/ethereum', function () {
  ...
  it('should scan for EIP-634 text records', async () => {
    ...
    for (const user of USERS) {
      for (const field of EIP634) {
        ...
        if (txt) {
          console.log(user.padEnd(20, ' '), field.padEnd(20, ' '), txt)
        }
      }
    }
  })
  ...
  it('should get address for .eth ENS resolver contract', async () => {
    const addr = await ethereum.ensRegistry.resolver(ethereum.namehash('eth'))
    // should be 0x30200E0cb040F38E474E53EF437c95A1bE723b2B
    console.log(addr)
  })

  it('should get registrar address from resolver contract', async () => {
    const addr = await ethereum.ensResolver.addr(ethereum.namehash('eth'))
    console.log(addr)
  })
  ...
  it('should resolve EIP-1185 DNS records from ENS', async () => {
    ...
    const record = await ethereum.resolveDnsFromEns(name, wire.types.A)
